...
const CAM = [
  ...
  [0.16, 0.08, 1.22, 2.15, 0.0, 1.12, -1.4],
  [0.34, -0.35, 1.32, 1.55, -0.9, 1.22, -2.2],
  [0.5, 0.05, 1.28, 1.35, 0.15, 1.2, -2.35],
  [0.66, 0.55, 1.3, 1.5, 1.05, 1.22, -2.2],
  [0.82, 0.0, 1.18, 1.9, 0.0, 1.08, -0.8],
  [1.0, 0.0, 1.55, 3.4, 0.0, 1.15, -0.2],
];
  ...
  function World({ reduced, mobile, cheap, getScroll }) {
    ...
    useFrame((state) => {
      ...
      if (focus >= 0 && PANS[focus] != null && p > 0.28 && p < 0.78) {
        look.x += (PANS[focus] - look.x) * 0.08;
        look.z += (-2.15 - look.z) * 0.06;
        pz += (0.55 - pz) * 0.04;
      }
